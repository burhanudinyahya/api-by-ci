...
class m_main_member extends CI_Model {
    ...
    public function to_insert($post) {
        ...
        $input_members = array(
            ...
            'f_member_name' => @$post['f_member_name'][0],
            'f_member_dob' => @$post['f_member_dob'][0],
            'f_member_gender' => @$post['f_member_gender'][0],
            'f_member_relation' => 1,
        ...
        foreach (@$post['f_benefit_id_1'] as $key => $value) {
            $premi = $this->_get_premi(@$post['f_plan_id_1'][$key],@$post['f_benefit_id_1'][$key],1,'premi');
            $curr = $this->_get_premi(@$post['f_plan_id_1'][$key],@$post['f_benefit_id_1'][$key],1,'curr');
            
            $total_premi = $total_premi + $premi;
            
            $input_flexis = array(
                'f_member_id' => $member_id,
                'f_benefit_id' => @$post['f_benefit_id_1'][$key],
                'f_plan_current' => @$post['f_plan_id_1'][$key],
                'f_plan_choice' => @$post['f_plan_id_1'][$key],
                'f_curr_choice' => @$curr,
